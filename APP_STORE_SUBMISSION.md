...
Practical checklist + copy-paste metadata for App Store Connect. Updated 2026-07-03.

---

## 0. Build prep (do this after pulling latest)

The iOS shell packages the built React app through Capacitor, so the native
project must be regenerated before every archive:

```bash
npm install                # installs Capacitor deps
npm run ios:sync           # builds React, then regenerates iOS assets/config/pods
```

Then in Xcode **once**:
- Confirm `ios/App/App/PrivacyInfo.xcprivacy` is present in the **App target**
  resources and appears at the root of the built `.app` bundle. Capacitor
  includes its own SDK manifests, but the app-level manifest must also ship.
- Confirm **Signing & Capabilities** has the **Push Notifications** capability
  enabled for `com.friedsports.app`.
- Set **Version** (e.g. 1.0.0) and **Build** (e.g. 1).

`ios/App/App/public` and `ios/App/App/capacitor.config.json` are generated by
Capacitor and ignored by Git. Run `npm run ios:sync` before opening/archive
building in Xcode so the native app ships the latest bundled React assets.

For TestFlight/App Store push notifications, Apple Developer and Railway must
agree on the same bundle/app identity:
- Apple Developer → Identifiers → `com.friedsports.app` → enable **Push Notifications**.
- Apple Developer → Keys → create an APNs key, download the `.p8` once, and save
  its Key ID.
- Railway env vars:
  - `APNS_KEY_ID`
  - `APNS_TEAM_ID`
  - `APNS_KEY_CONTENT` with the full `.p8` contents
...
Notes: Email + password signs you straight in (no email code needed). The
account is already profile-complete and has a populated group/thread. After
login, tap "Groups" to view groups, open an active thread, and post a message.
To report content, long-press another user's message, choose "Report message",
then pick a category. You can also block users from that report sheet or from
the friends list overflow menu.
```

> Make sure this is a non-admin production account in at least one group with an
> active thread containing another user's message, so the reviewer can verify
> report/block without needing to create extra accounts.
...
**Current mitigation:** the app now ships bundled React assets from
`frontend/dist` via Capacitor's `webDir`; `capacitor.config.json` no longer sets
`server.url`. The native bundle calls `https://www.friedsports.com/api` for
backend data, so the binary is not just a WebView pointed at the live website.

The app also uses real native iOS capabilities a website can't, via
`app/static/js/native.js`:
...
a pass. If rejected, reply in Resolution Center pointing to the bundled assets,
native share sheet, haptics, native splash screen, status bar, and app icon.
...
**Reviewer note:** Message reporting is in the long-press message action sheet,
not a permanently visible icon. This keeps the chat UI clean but should be
called out in App Review notes.

---

## 7. Final pre-submit checklist

- [ ] `npm run ios:sync` run after the latest frontend changes
- [ ] PrivacyInfo.xcprivacy is in the App target and built `.app` root
- [ ] Push Notifications capability enabled for `com.friedsports.app`
- [ ] APNs key/env vars configured in Railway with `APNS_ENV=production`
- [ ] Review account created in prod + credentials in App Review notes
- [ ] Review account is non-admin, profile-complete, and has a populated thread
